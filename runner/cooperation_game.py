...
def play_cooperation_game(c1: str, c2: str) -> Tuple[int, int]:
    """
    ...
    for _ in range(_Config.nb_rounds):
        try:
            decision_1 = _decode_output(
                client.containers.run(c1, _encode_input(moves_1, moves_2))
            )
        ...
        try:
            decision_2 = _decode_output(
                client.containers.run(c2, _encode_input(moves_2, moves_1))
            )
        except ContainerError:
        ...
        if [decision_1, decision_2] == ["B", "B"]:
            ...
            print(
                f"{c1} betrays {c2} and wins {_Config.reward_on_sole_betrayer} point(s)"
            )
            score_1 += _Config.reward_on_sole_betrayer
        elif [decision_1, decision_2] == ["C", "B"]:
            print(
                f"{c2} betrays {c1} and wins {_Config.reward_on_sole_betrayer} point(s)"
            )
